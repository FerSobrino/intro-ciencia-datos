...
\begin{center}
\includegraphics[width=0.62\textwidth]{../images/anscombe.png}
\end{center}

## El cuarteto de Anscombe: ¿qué estamos viendo?
...
- El orden si importa: primero mirar, después resumir. 

# La distribución de una variable

## Una variable sola: conteos e histogramas

**Categóricas** — primero contar cuantas cosas hay por categoría 

- ¿El orden es el esperado? Las **ordinales hay que declararlas** como tales, o `pandas` las ordena alfabéticamente
- Si tienes muchas categorías la gráfica no va a servir: agrupa la cola larga en `otros`
- Ojo con promediar categorías: si codificas [1, 2, 3] y sacas promedio, puedes obtener 2 aunque nadie haya escogido 2


## Una variable sola: conteos e histogramas

**Continuas** — un histograma es el primer paso

- Lo que buscamos es que la distribución de vea como esperamos que sea 
- Si tiene alguna clase de señal: picos, tendencias centrales, colas pesadas
- Ojo: los datos continuos no tienen que distribuirse como una normal
- En la naturaleza casi ninguna variable se va a ver así 
...
- Los mismos datos con `bins` por defecto y con `bins=50`
...
- **Mediana**: el valor de en medio; no le importan los extremos 
- **Moda**: la única que tiene sentido para datos ordinales o categóricos
- Regla práctica: si media y mediana difieren mucho, la variable es sesgada
...
## Valores extremos: qué hacer con ellos

Tres opciones, todas tienen consecuencias:

- **Tirarlos** — pierdes información y sesgas si el extremo es real
- **Convertirlos en faltantes** — conservas el renglón, marcas la duda
- **Winsorizar** — recortar al percentil 1 y 99
...
## Un faltante no siempre va a ser un NaN

- Revisa siempre el diccionario de datos: no es lo mismo un dato faltante, un "No aplica" y un cero
- Los códigos centinela (`-99`, `999`, `-1`, `""`, `"N/A"`) entran como números y van a contaminar el análisis si no te das cuenta
...
- **MNAR** (*not at random*): la falta depende del valor no observado. No responden ingreso justamente los de ingreso alto. No se puede diagnosticar con los datos, requiere conocimiento del tema

- Se diagnostica comparando a los que sí contestaron contra los que no, en las variables que sí tienes. Eso distingue MCAR de MAR pero nunca MNAR

## Por qué no hay que borrar los NaN

- `dropna()` puede parecer lo primero que hay que hacer pero hay dos problemas con esto:
...
2. Pearson solo ve líneas: una U perfecta tiene correlación ≈ 0
...
## Qué verificar cuando un agente hace EDA
