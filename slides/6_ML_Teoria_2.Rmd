...
- KNN contesta: busca los $k$ casos más parecidos que ya conoces y asume que se parece a sus vecinos(voto mayoritario)
...
- El codificador también se aprende **solo con train** — y hay que decidir qué hacer con una categoría que solo aparece en test
...
## La misma persona, dos hojas distintas

\begin{center}
\includegraphics[height=2.5cm]{../images/ozzy_osbourne.jpg}\hspace{2em}
\includegraphics[height=2.5cm]{../images/ozzy_osbourne.jpg}

\vspace{0.2em}
\small 59 años, 364 días → «riesgo bajo» \hspace{2.5em} 60 años, 0 días → «riesgo alto»
\end{center}

- Un corte es una frontera dura: con ¿edad $\geq$ 60?, la misma persona cae en hojas opuestas con un día de diferencia
- Es el espejo de la trampa de KNN: KNN convierte a dos personas distintas en el mismo punto; el árbol convierte a la misma persona en dos casos opuestos
- Casi nada en el mundo cambia de golpe en un umbral, pero el árbol solo sabe dibujar escalones: cerca del corte, desconfía de la predicción
...
- Si el bosque no le gana claramente al árbol, quédate con el árbol

## ¿Cómo entender esto de manera intuitiva? 

- En *¿Quién quiere ser millonario?* había dos comodines: llamar a un amigo experto o preguntarle al público
- El amigo experto acertaba cerca del **65%** de las veces; el público, cerca del **91%**
- Muchas opiniones mediocres pero diversas, promediadas, le ganan a una sola opinión inteligente
- Eso es un Random Forest: cada árbol es un miembro del público que vio datos distintos y features distintas
- La condición escondida: los errores deben ser independiente. Si todo el público repite el mismo rumor — o todos los árboles dependen de la misma columna dominante — el promedio no arregla nada. Por eso el bosque fabrica la diversidad a propósito
...
- Cómo leerlo: $\exp(\text{coef})$ es el **odds ratio** — 1.4 significa "los odds suben 40% por unidad". Comparar coeficientes entre sí solo vale si las variables están escaladas, y un coeficiente **no** es un efecto causal
- Sale un número entre 0 y 1 — y para actuar hace falta elegir un corte
...
- Es naive (ingenuo) porque supone que las features son independientes entre sí. Casi nunca es cierto. 
- Aun así funciona sorprendentemente bien, es rápido y sirve con pocos datos
...
- Empieza por la logística, y compara siempre contra una **línea base tonta**: predecir la clase mayoritaria
- Nota la columna de escalar: en KNN es obligatorio, en árboles irrelevante — y en ambos casos el código corre igual
