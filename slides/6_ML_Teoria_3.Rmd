...
- Un número solo no dice nada. Un 92% puede ser un modelo excelente o un modelo peor que no hacer nada
...
- Si el objetivo es no dejar pasar riesgos, es un mal modelo. Si el objetivo es no molestar de más, no es tan malo
...
## Eficiencia y cobertura (cont.)

- **Precisión** $= \dfrac{VP}{VP+FP}$: de los que señalé, ¿cuántos eran? Es la **eficiencia** del recurso
- **Cobertura** (*recall*) $= \dfrac{VP}{VP+FN}$: de los que había, ¿a cuántos encontré? Es la **cobertura** del problema
...
- **Nunca reportes una sin la otra.** Un agente que solo te da precisión te está dando media historia
...
- Su defecto: le da **el mismo peso** a los dos errores. Si en tu problema un FN cuesta diez veces más que un FP, F1 está mintiendo por construcción
- Úsalo para ordenar candidatos, no para decidir
...
- Se lee así: "si quiero 60% de cobertura, mi precisión será de 81%". Esa frase sí es accionable
- Con clases muy desbalanceadas, esta curva dice mucho más que la ROC
...
- Mide qué tan bien el modelo **ordena** los casos: la probabilidad de que un positivo tomado al azar tenga mayor puntaje que un negativo
- Es independiente del umbral y de la escala — y ahí está la trampa: **ignora el costo asimétrico de los errores**
- Con una clase al 2%, un AUC de 0.85 puede convivir con una precisión pésima en los casos que sí vas a atender
- Sirve para comparar modelos entre sí. **No** sirve para decidir a quién mandas un inspector
...
- Si el agente te reporta accuracy en un problema con presupuesto, está contestando otra pregunta
...
- Esa regla no vive en el modelo. Vive en la política, en el presupuesto y en la ley

## Elegir el umbral no es estadística
...
- Esa decisión no la puede tomar el modelo, ni quien lo programó, ni el agente de IA: la toma **quien responde por el programa**
...
## La misma métrica, distinta por grupo (cont.)
